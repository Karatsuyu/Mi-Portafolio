...
// Mantener 5 “lunas” en órbita para preservar el estilo actual.
const ORBIT_CONTACTS: ContactItem[] = [
  {
    id: "github",
    label: "GitHub",
    color: "#c084fc",
  ...
  },
  {
    ...
    color: "#4ade80",
    glowColor: "#22c55e",
    href: "https://wa.me/573234378971",
    svgIcon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        <path d="M20.52 3.48A11.82 11.82 0 0012.02 0C5.4 0 .02 5.38.02 12c0 2.11.55 4.17 1.6 5.98L0 24l6.23-1.6A11.93 11.93 0 0012.02 24C18.64 24 24 18.62 24 12c0-3.2-1.25-6.2-3.48-8.52zM12.02 21.8c-1.84 0-3.64-.5-5.21-1.45l-.37-.22-3.7.95.98-3.6-.24-.37A9.72 9.72 0 012.22 12c0-5.4 4.4-9.8 9.8-9.8 2.62 0 5.08 1.02 6.93 2.87a9.74 9.74 0 012.87 6.93c0 5.4-4.4 9.8-9.8 9.8zm5.7-7.17c-.31-.16-1.82-.9-2.1-1-.28-.1-.49-.16-.7.16-.2.31-.8 1-.98 1.2-.18.2-.36.23-.67.08-.31-.16-1.31-.48-2.49-1.52-.92-.82-1.54-1.83-1.72-2.14-.18-.31-.02-.48.13-.64.14-.14.31-.36.47-.54.16-.18.2-.31.31-.52.1-.2.05-.39-.03-.55-.08-.16-.7-1.68-.96-2.3-.25-.6-.5-.52-.7-.53h-.6c-.2 0-.52.08-.8.39-.28.31-1.05 1.03-1.05 2.52 0 1.49 1.08 2.93 1.23 3.13.16.2 2.12 3.24 5.14 4.54.72.31 1.28.5 1.72.64.72.23 1.38.2 1.9.12.58-.09 1.82-.74 2.08-1.45.26-.71.26-1.31.18-1.45-.08-.14-.28-.23-.6-.39z"/>
      </svg>
    ),
  },
];

// Contactos extra (no cambian el layout orbital; aparecen en el menú radial).
const EXTRA_CONTACTS: ContactItem[] = [
  {
    id: "email",
    label: "Email",
    color: "#facc15",
  ...
  },
  {
    ...
    color: "#22d3ee",
    glowColor: "#06b6d4",
    href: "https://www.google.com/maps/place/El+Vergel,+G%C3%A9nova,+Quind%C3%ADo/@4.1953196,-75.8069597,14z",
    svgIcon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
...
const ALL_CONTACTS: ContactItem[] = [...ORBIT_CONTACTS, ...EXTRA_CONTACTS];
      ...
      <defs>
        {ORBITAL_RINGS.map((ring, i) => (
          <linearGradient key={`grad-${i}`} id={`ringGrad${i}`} x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor={ORBIT_CONTACTS[i]?.color ?? "#c084fc"} stopOpacity="0.05" />
            <stop offset="50%" stopColor={ORBIT_CONTACTS[i]?.color ?? "#c084fc"} stopOpacity="0.35" />
            <stop offset="100%" stopColor={ORBIT_CONTACTS[i]?.color ?? "#c084fc"} stopOpacity="0.05" />
          </linearGradient>
        ))}
        <filter id="ringGlow">
      ...
      {ORBITAL_RINGS.map((ring, i) => (
        <g key={i} transform={`rotate(${ring.rotation})`}>
          ...
          <ellipse
            ...
            ry={ring.radiusY + 3}
            fill="none"
            stroke={ORBIT_CONTACTS[i]?.color ?? "#c084fc"}
            strokeWidth="0.3"
            opacity="0.15"
          >
            <animateTransform
...
function RadialMenu({
  ...
  const angleStep = (2 * Math.PI) / ALL_CONTACTS.length;
            ...
            return (
              <motion.div
                ...
                exit={{
                  scale: 0,
                  opacity: 0,
                  transition: { duration: 0.2, delay: (ALL_CONTACTS.length - 1 - i) * 0.03 },
                }}
                transition={{
                  duration: 0.5,
                  delay: 0.15 + i * 0.08,
...
const Contacto = () => {
  ...
  const activeMoon = ORBIT_CONTACTS.find(m => m.id === hoveredMoon) ?? null;

  const handleMoonClick = (id: string) => {
    const moon = ALL_CONTACTS.find(m => m.id === id);
