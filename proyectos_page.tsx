...
import CyberProjectModal, { CYBER_PROJECTS, type CyberProject } from '@/components/cyber/proyectos/CyberProjectModal';
...
export default function ProyectosPage() {
  ...
  const [modalProject, setModalProject] = useState<CyberProject | null>(null);

  const openModal = (project: Project) => {
    // Busca el proyecto completo en CYBER_PROJECTS por id o title
    const full = CYBER_PROJECTS.find(p => 
      p.id === project.id || 
      p.title.toLowerCase() === project.title.toLowerCase()
    );
    if (full) setModalProject(full);
    else setSelectedProject(project); // fallback al panel original
  };
  ...
  return (
    <>
      ...
      {/* Panel de detalle fallback (proyectos sin data completa) */}
      <AnimatePresence>
      ...
      {/* ── NUEVO MODAL COMPLETO CON SCC + MÉTRICAS + ARQUITECTURA ── */}
      <AnimatePresence>
        {modalProject && (
          <CyberProjectModal
